...
and if those datasets were previously annotated internally.
"""
    ...
    for json_file in json_files:
        ...
        for column_contents in contents.values():
            if isinstance(column_contents, dict) and column_contents.get("Annotations"):
                ...
                break  # No need to check other columns in this file

    if unparsable:
        print(
            f"Unparsable JSON files ({len(unparsable)}/{len(json_files)}): {', '.join(unparsable)}"
        )
    print(
